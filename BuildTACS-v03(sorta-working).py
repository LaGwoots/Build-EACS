...
def last_user():
        with open('/home/pi/Desktop/RFID/user_list') as users:
            ...
            for row in users:
                if member_type_list[pos] == 'table_saw' > 0 and relayState == 0:
                    last = row[0]
                    log_file = open('/home/pi/Desktop/RFID/Log/user_list.csv', 'r')
                    new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + last[rfid_number] + ',' + names_list[pos] + ',Turned on machine \n')
                    print(new_line)
                    contents = log_file.readlines()
                    contents.insert(0, new_line)
                    log_file.close()
                    log_file = open('/home/pi/Desktop/RFID/Log/user_list.csv', 'w')
                    log_file.writelines(contents)
                    log_file.close()
                    print(last + 'last')
                if member_type_list[pos] == 'table_saw' > 0 and relayState == 1:
                    current = row[1]
                    log_file = open('/home/pi/Desktop/RFID/Log/user_list.csv', 'r')
                    new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + current[rfid_number] + ',' + names_list[pos] + ',Turned off machine \n')
                    print(new_line)
                    contents = log_file.readlines()
                    contents.insert(0, new_line)
                    log_file.close()
        ...
        if member_type_list[pos] == 'table_saw' > 0 and relayState == 0 and last[rfid_number] == current[rfid_number]:
            machine_turnoff()
        if member_type_list[pos] == 'table_saw' > 0 and relayState == 0 and last[rfid_number] != current[rfid_number]:
            log_file = open('/home/pi/Desktop/RFID/Log/left_on.csv', 'r')
            new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + last + ' left the machine on \n')
            print(new_line)
            contents = log_file.readlines()
            contents.insert(0, new_line)
            log_file.close()
